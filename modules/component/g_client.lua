...
function jo.component.applyComponents(ped, components)
  ...
  if type(outfit) == "table" and next(outfit) then
    -- Trava: tabela só de zeros/nil não é traje (ex.: slot esvaziado) —
    -- sem isto, cairia no return e o ped ficava pelado no revive/login.
    local hasPiece = false
    for subCat, subData in pairs(outfit) do
      if subCat ~= "outfits" and type(subData) == "table"
        and (subData.drawable or subData.hash or subData.wearableState) then
        hasPiece = true
        break
      end
    end
    if hasPiece or (type(outfit.outfits) == "table"
      and (outfit.outfits.drawable or outfit.outfits.hash)) then
      for subCat, subData in pairs(outfit) do
